...
import io
import base64
from utils.session_state import get_session_state, log_message, update_progress, register_chart
...
""")
    ...
    with viz_tabs[0]:
        ...
        with st.spinner("正在生成词云..."):
            ...
            # 注册图表
            from utils.session_state import export_chart_to_base64
            img_base64 = export_chart_to_base64(st.session_state.wordcloud_images[cache_key])
            if img_base64:
                topic_keywords = st.session_state.get("topic_keywords", {})
                keywords = topic_keywords.get(topic_id, [])[:5]
                register_chart(
                    chart_id=f"wordcloud_topic_{topic_id}",
                    chart_type="matplotlib",
                    title=f"主题 {topic_id + 1} 词云",
                    description=f"主题 {topic_id + 1} 的Top {max_words}高频词，关键词: {', '.join(keywords)}",
        ...
        if st.button("生成所有主题的词云", key="gen_all_wordclouds"):
            with st.spinner("正在生成所有主题的词云..."):
                ...
                # 注册所有主题词云
                from utils.session_state import export_chart_to_base64
                img_base64 = export_chart_to_base64(fig)
                if img_base64:
                    register_chart(
                        chart_id="wordcloud_all_topics",
                        chart_type="matplotlib",
                        title="所有主题词云汇总",
                        description=f"包含 {n_topics} 个主题的词云可视化",
                        image_base64=img_base64
                    )
    ...
    with viz_tabs[1]:
        ...
        with st.spinner("正在生成文档-主题分布热图..."):
            ...
            if img_base64:
                register_chart(
                    chart_id="doc_topic_heatmap",
                    chart_type="plotly",
                    title="文档-主题分布热图",
                    description=f"展示 {len(df)} 个文档在各主题上的概率分布",
                    image_base64=img_base64
                )
