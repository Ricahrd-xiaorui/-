...
def load_example_data():
    """加载示例政策数据（优先从temp/examples目录读取）"""
    # 定义示例数据目录路径（使用绝对路径）
    example_dir = r"D:\project\lda-1\temp\examples"
        ...
        for filename in os.listdir(example_dir):
            ...
            if os.path.isfile(file_path) and is_supported_file(filename):
                ...
                if content and content.strip():
                    file_contents[filename] = content.strip()
                    file_paths.append(file_path)
        
        if file_contents:
            log_message(f"从 {example_dir} 加载了 {len(file_contents)} 个示例文件", level="info")
            return file_paths, file_contents
    
    # 如果目录不存在或为空，使用内置示例数据
    log_message(f"示例目录 {example_dir} 不存在或为空，使用内置示例数据", level="info")
