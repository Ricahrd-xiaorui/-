...
def get_project_root():
    """获取项目根目录的绝对路径"""
    from config.system import PROJECT_ROOT
    return PROJECT_ROOT
...
def render_text_processor():
    ...
    with st.expander("📚 专业词典管理", expanded=False):
        ...
        **专业词典** 用于提高分词准确性，确保专业术语被正确识别。
        ...
        ✅ **系统已内置通用政策词典**，包含常用政策术语，将自动加载。

        💡 **提示**：如需添加其他专业术语，可在下方自定义添加。可从以下渠道获取：
        - 学术论文中的关键词列表
        - 行业标准术语表
        - 政策文件中的专有名词
        - 搜狗细胞词库等公开资源
        ...
        try:
            ...
            if st.session_state.get("dictionary_manager") is None:
                ...
                default_dict_path = "通用政策专业词典.txt"
                if os.path.exists(default_dict_path):
                    dm = st.session_state["dictionary_manager"]
                    success, msg = dm.import_from_file(
                        "通用政策词典",
                        default_dict_path,
                        "系统内置通用政策专业术语词典"
                    )
                    if success:
                        log_message(f"已自动加载通用政策词典: {msg}", level="info")
    ...
    with st.expander("停用词管理", expanded=True):
        ...
        with tabs[2]:
            if current_stopwords:
                ...
                st.dataframe(stopwords_df, width='stretch', height=300)
    ...
    if st.session_state.texts and st.session_state.dictionary and st.session_state.corpus:
        ...
        with st.expander("词频统计", expanded=False):
            ...
            # 显示词频表格
            st.dataframe(df_word_counts, width='stretch')
        ...
        with st.expander("文档长度分布", expanded=False):
            ...
            # 显示文档长度表格
            st.dataframe(df_lengths.sort_values("词语数量", ascending=False), width='stretch')
