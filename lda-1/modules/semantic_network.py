...
def _render_network_visualization(builder, max_nodes: int):
    ...
    if num_nodes == 0:
        st.warning("网络为空，无法可视化")
        return
    
    st.info(f"📊 当前网络：{num_nodes} 个节点，{num_edges} 条边（将显示前 {max_nodes} 个节点）")
    
    if num_nodes > 200:
        st.warning(f"⚠️ 网络较大，建议：\n- 提高最小边权重以减少节点数\n- 使用核心概念词过滤\n- 或减少最大节点数")
    
    with st.expander("🎨 可视化设置", expanded=True):
        vis_col1, vis_col2, vis_col3 = st.columns(3)
        
        with vis_col1:
            layout_algorithm = st.selectbox("布局算法", ["spring (力导向)", "circular (圆形)", "shell (同心圆)", "random (随机)"], index=0, key="semantic_layout_algo")
        
        with vis_col2:
            color_by = st.selectbox("节点颜色依据", ["度数", "社区", "度中心性", "介数中心性"], index=0, key="semantic_color_by")
        
        with vis_col3:
            show_labels = st.checkbox("显示节点标签", value=num_nodes <= 50, key="semantic_show_labels")
    
    # 添加生成按钮
    if st.button("🎨 生成网络图", key="generate_semantic_viz", type="primary"):
        with st.spinner("正在生成网络图..."):
            try:
                ...
                st.info(f"准备渲染 {len(nodes)} 个节点，{len(edges)} 条边")
                ...
                if color_by == "社区":
                    communities = st.session_state.get("community_labels", {})
                    if not communities:
                        st.info("💡 需要先进行社区检测才能按社区着色，当前使用度数着色")
                        color_by = "度数"
                elif color_by in ["度中心性", "介数中心性"]:
                    centrality = st.session_state.get("centrality_metrics", {})
                    if not centrality:
                        st.info("💡 需要先计算中心性指标才能使用该着色方式，当前使用度数着色")
                        color_by = "度数"
                ...
                for node_data in nodes:
                    ...
                    if node_id in pos:
                        ...
                        # 构建悬停文本
                        hover_parts = [f"{node_id}", f"度数: {degree}"]
                ...
                st.error(f"需要安装plotly库才能显示网络图: pip install plotly")
